<template>
    <div :style="{'--imgHeight': props.imgHeight, '--imgWidth': props.imgWidth}">
        ...
        >
            ...
            <el-table-column v-if="props.tableColumnIndex" type="index" width="55" />
            <el-table-column v-for="item in tableColumn" :key="item.prop" :prop="item.prop" :label="item.label" :width="item.width" :align="item.align || 'center'" :sortable="item.sortable" :sort-method="item.isCustomSort ? (a:any,b:any)=>customSort(a,b,item.prop) : undefined">
                <template #default="{ row }">
                    ...
                    <el-image v-else-if="item.img" :src="row[item.prop]" class="imgStyle" :preview-src-list="item.list || [row[item.prop]]" preview-teleported show-progress>
                        <template #error>
                    ...
                    <!-- 选择框 -->
                    <el-select v-else-if="item.select" v-model="item.prop" @change="emit('selectChange', $event, row)" :placeholder="item.placeholder || '请选择'" clearable style="width: 100%">
                        <el-option
                    ...
                    <el-tag v-else-if="item.tag" :type="item.statusObj[row[item.prop]].type">{{ item.statusObj[row[item.prop].text ] }}</el-tag>
                    
                    <div v-else :style="{color: item.color || '#000'}">{{ row[item.prop] }}</div>
                </template>
            </el-table-column>
        </el-table>
...
const props = defineProps({
    ...
    headerStyle: { // 表头样式 例子 { background: '#f5f7fa', color: '#6B6B86' }
        type: Object,
    ...
    paginationClass: { // 分页类名（暴露给父组件的分页类名，可以直接写样式，记得是在没有scope里面style里面写，否则需要样式穿透）
        type: String,
        default: ''
    },
    rowConditionChangeColorArr:{ // 行条件改变颜色数组
...
const tableRowClassName = ({ row }: any) => {
    ...
    if(props.rowConditionChangeColorArr.length > 0){
        const colorRow = props.rowConditionChangeColorArr.find(item=> row[item.condition] === item.value)
